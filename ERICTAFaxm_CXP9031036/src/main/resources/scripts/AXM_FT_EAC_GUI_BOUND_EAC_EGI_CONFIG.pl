...
#
################################ Description ######################
#This test case checks that EAM client eac_egi_config fetches proper authority 
...
#my $TC         = "AXM_FT_EAC_GUI_BOUND_EAC_EGI_CONFIG_NO_NODE";
#my $RESULT_DIR = $ENV{RESULT_DIR};
#my $TMP        = $ENV{TMP};

#$ENV{tc} = AXM_FT_EAC_GUI_BOUND_EAC_EGI_CONFIG_NO_NODE;
...
open(RFH,">AXM_FT_EAC_GUI_BOUND_EAC_EGI_CONFIG_NO_NODE.txt");
   ...
   system("/opt/ericsson/bin/eac_egi_config -l > eac_egi_config_nmsadm.log");
  
@size_search = `/bin/egrep "TMOS_node\|FILE_NOTIFICATION_ON" eac_egi_config_nmsadm.log`;
...
#   Te::tex "$TC","INFO  : User Already Exists";
  ...
  dummy;
  system("/bin/cat eac_egi_config_dummy.log | /bin/grep not > /dev/null");
...
system("/bin/cat AXM_FT_EAC_GUI_BOUND_EAC_EGI_CONFIG_NO_NODE.txt | /bin/grep FAIL ");
if ($? == 0)
{
print  "FAIL\n";
}
...
{
print "\nPASS\n";
#unlink("AXM_FT_EAC_GUI_BOUND_EAC_EGI_CONFIG_NO_NODE.txt");
}
