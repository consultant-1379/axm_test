#!/use/bin/perl
#
# SCRIPT NAME:AXM_FT_EAC_GUI_BOUND_EAC_EGI_CONFIG_NO_NODE.pl
# Test Case & Priority: AXM_FT_EAC_GUI_BOUND_EAC_EGI_CONFIG_NO_NODE (Pr.1)
# Test Case No :5.2.3
# AUTHOR: XNNNKKR
# DATE  :02/05/2013
# REV:1.0
#
# Description :Verifying the eac_sb_adm -l command execution using nmsadm and DUMMY(Void of command execution permission) user.
# Return Value on Success : PASS
# Return Value on Failure : FAIL
#
# Usage :  /usr/local/bin/perl AXM_FT_EAC_GUI_BOUND_EAC_EGI_CONFIG_NO_NODE.pl.
#
# Dependency :root user password should be tds1439
#
# REV HISTORY
...
open(RFH,">AXM_FT_EAC_GUI_BOUND_EAC_EGI_CONFIG_NO_NODE.txt");
sub execution_nmsadm
{
	print "\nExecuting for nmsadm\n";
   	system("/opt/ericsson/bin/eac_egi_config -l > /home/nmsadm/eac_egi_config_nmsadm.log");
  
	@size_search = `/usr/bin/egrep "TMOS_node\|FILE_NOTIFICATION_ON" /home/nmsadm/eac_egi_config_nmsadm.log`;
...
sub cmd
{
	print "\nExecuting for DUMMY\n";
 	$expect = Expect->spawn("su - root");
        ...
        $expect->log_file("/home/nmsadm/eac_egi_config_dummy.log","w");
    	...
    	$expect->expect (5,'>'); 
    	$expect->send("/opt/ericsson/bin/eac_egi_config -l\r");
    	$expect->expect (10,'1>');
    	$expect->soft_close();
} 
...
$expect3->log_file("/home/nmsadm/eac_egi_config_user_creation_mas.log","w");
...
$expect1->log_file("/home/nmsadm/eac_egi_config_user_creation_uas.log","w");
...
system("/usr/bin/grep not /home/nmsadm/eac_egi_config_dummy.log && /usr/bin/grep TSS /home/nmsadm/eac_egi_config_dummy.log");
...
close(RFH);
system("/usr/bin/grep FAIL AXM_FT_EAC_GUI_BOUND_EAC_EGI_CONFIG_NO_NODE.txt");
if ($? == 0)
{
	print  "FAIL\n";
}
