#!/usr/bin/perl
#
# SCRIPT NAME:AXM_FT_EAC_PORT_eac_sb_server_NO_NODE.pl
# Test Case &Priority:AXM_FT_EAC_PORT_eac_sb_server_NO_NODE.pl (Pr.1)
# Test Case No :6.11.8(common)
# AUTHOR:
# DATE  :
# REV:
#
...
#
# Usage :  bash RUNME -t AXM_FT_EAC_PORT_eac_sb_server_NO_NODE.pl:NO_NODE.
#
...
#my $TC         = "AXM_FT_EAC_PORT_eac_sb_server_NO_NODE";
#my $RESULT_DIR = $ENV{RESULT_DIR};
#my $TMP        = $ENV{TMP};

#$ENV{tc} = AXM_FT_EAC_PORT_eac_sb_server_NO_NODE;
$ntwrk_element=$ARGV[0];#$ENV{ntwrk_element};

#FRAME::start_frame "$TC";
#======================================================================
#open(RFH,">AXM_FT_EAC_PORT_eac_sb_server_NO_NODE.txt");
open RFH, ">AXM_FT_EAC_PORT_eac_sb_server_NO_NODE.txt" or die $!;
sub Common_Restart
{
...
print "#######################\n";
$cmd = `/usr/sbin/makedbm -u /etc/opt/ericsson/nms_cif_tmos_tbs/dat/.ipcdir_ndbm_table | /bin/grep 50006`;

if ($cmd =~ /masterservice eac_sb_server_appl 50006/)
{
...
system("perl -pi -e 's/EAC_SUBS_PORT=50006/#EAC_SUBS_PORT=50006/g' /etc/opt/ericsson/nms_eam_eac/cxc.env");
sleep(5);
Common_Restart;
print "Port No value After comment \n";
print "#######################\n";
$cmd1 = `/usr/sbin/makedbm -u /etc/opt/ericsson/nms_cif_tmos_tbs/dat/.ipcdir_ndbm_table | /bin/grep eac_sb_server_appl`;
chomp($cmd1);

if ($cmd1 =~  /masterservice eac_sb_server_appl 50006/)
{
...
sleep(5);
Common_Restart;
$cmd2 = `/usr/sbin/makedbm -u /etc/opt/ericsson/nms_cif_tmos_tbs/dat/.ipcdir_ndbm_table | /bin/grep eac_sb_server_appl`;
print "$cmd2\n";
if ($cmd2 =~ /masterservice eac_sb_server_appl 50006/)
{
...
system("/bin/cat AXM_FT_EAC_PORT_eac_sb_server_NO_NODE.txt | /bin/grep FAIL ");
if ($? == 0)
{
print  "\nFAIL\n";
}
...
{
print "\nPASS\n";
#unlink("AXM_FT_EAC_PORT_eac_sb_server_NO_NODE.txt");
}
