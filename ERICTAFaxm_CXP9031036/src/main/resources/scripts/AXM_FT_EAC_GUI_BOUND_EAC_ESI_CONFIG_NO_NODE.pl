#!/use/bin/perl
#
# SCRIPT NAME:AXM_FT_EAC_GUI_BOUND_EAC_ESI_CONFIG_NO_NODE.pl
# Test Case & Priority: AXM_FT_EAC_GUI_BOUND_EAC_ESI_CONFIG_NO_NODE (Pr.1)
# Test Case No :5.2.5
# AUTHOR: XNNNKKR
# DATE  :03/05/2013
# REV:1.0
...
# Usage :  /usr/local/bin/perl AXM_FT_EAC_GUI_BOUND_EAC_ESI_CONFIG_NO_NODE.pl.
#
...
sub execution_nmsadm
{
	...
	system("/opt/ericsson/bin/eac_esi_config -nl > /home/nmsadm/eac_esi_config_nmsadm.log");
  	system("/usr/bin/grep NAME /home/nmsadm/eac_esi_config_nmsadm.log && /usr/bin/grep PROTOCOL /home/nmsadm/eac_esi_config_nmsadm.log");
	if ( $? == 0 ) 
	{
...
sub cmd
{
 	print "\nExecuting for DUMMY\n";
  	...
  	$expect->log_file("/home/nmsadm/eac_esi_config_dummy.log","w");
  	...
  	$expect->expect (5,'>'); 
  	$expect->send("/opt/ericsson/bin/eac_esi_config -nl\r");
  	$expect->expect (10,'1>');
  	$expect->soft_close();
} 
	...
	system(" ls -lrt /home/ | /usr/bin/grep DUMMY");
  	if($? == 0)
  	{
  	...
  	else
   	{
  		...
  		$expect3->log_file("/home/nmsadm/eac_esi_config_user_creation_mas.log","w");
...
if($server =~ vts)
{
	...
	else
	{
		$expect1 = Expect->spawn("su - root");
  		$expect1->log_file("/home/nmsadm/eac_esi_config_user_creation_uas.log","w");
...
system("/usr/bin/grep not /home/nmsadm/eac_esi_config_dummy.log && /usr/bin/grep TSS /home/nmsadm/eac_esi_config_dummy.log");
...
system("/usr/bin/grep FAIL AXM_FT_EAC_GUI_BOUND_EAC_ESI_CONFIG_NO_NODE.txt");
if ($? == 0)
{
	print  "FAIL\n";
}
