...
# SCRIPT NAME:AXM_FT_EAM_ReturnPrintoutTitle_EHIP.pl
# Test Case & Priority:AXM_FT_EAM_ReturnPrintoutTitle_EHIP_S   (Pr.3)
...
#
######################### Description #######################
# This test case verifies the return printout title of caclp command in showinfo mode # 
#############################################################
# Return Value on Success : PASS
# Return Value on Failure : FAIL
# Usage :/usr/local/bin/perl AXM_FT_EAM_ReturnPrintoutTitle_EHIP.pl <node_name>
...
open(RFH,">AXM_FT_EAM_ReturnPrintoutTitle_EHIP.txt");
$key= "TIME";
$key1="Response complete";
...
$expect->log_file("/home/nmsadm/ReturnPrintoutTitle.log","w");
$expect->expect(10,'<');
...
@size_search = `/bin/egrep "$key\|$key1" /home/nmsadm/ReturnPrintoutTitle.log"`;
...
system("/bin/cat AXM_FT_EAM_ReturnPrintoutTitle_EHIP.txt | /bin/grep FAIL ");
