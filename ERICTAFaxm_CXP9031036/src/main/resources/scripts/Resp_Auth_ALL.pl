...
my $pro_node="";
my $i=0;
unlink("MML_CMDS_TELNET_NODES.txt");
...
print "Establishing connection for $pro_node\n";    
        ...
        $expect->log_file("Imm_ResponseAuth.log","w+");
        $expect->expect(5,'<');
        $expect->send("caclp;\r");
        $expect->expect(5,'<');
        $expect->send("quit;\r");
...
system("/usr/bin/grep TIME Imm_ResponseAuth.log >> /dev/null");

if ( $? == 0 ) 
{
 print RFH "$pro_node:PASS\n";
 unlink("Imm_ResponseAuth.log"); 
}
else 
{
...
system("cat MML_CMDS_TELNET_NODES.txt"); 
system("/usr/bin/grep FAIL MML_CMDS_TELNET_NODES.txt >> /dev/null");
